...
/*
 * Upstream keeps this in <linux/writeback.h>; this tree's f2fs and sdfat
 * carry private copies with the same name, so keep iomap's private too.
 */
...
{
	iomap_ioend_bioset = bioset_create(4 * (PAGE_SIZE / SECTOR_SIZE),
			offsetof(struct iomap_ioend, io_inline_bio),
			BIOSET_NEED_BVECS);
	return iomap_ioend_bioset ? 0 : -ENOMEM;
}
