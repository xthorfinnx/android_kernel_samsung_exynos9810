 ...
 * immediately before erofs's own conversion (f57a3fe44995) -- see
 * include/linux/fs_parser.h for why struct p_log isn't backported)
 */
	     ...
	     struct fs_parse_result *result)
{
	...
	if (p->flags & fs_param_deprecated)
		warnfc(fc, "Deprecated parameter '%s'", param->key);
	...
	if (is_flag(p)) {
		if (param->type != fs_value_is_flag)
			return invalfc(fc, "Unexpected value for '%s'",
				      param->key);
		result->boolean = !result->negated;
	} else  {
		int ret = p->type(fc, p, param, result);
		if (ret)
			return ret;
	}
	return p->opt;
...
/*
 * fs_lookup_param() (path-typed mount parameter lookup, e.g. fsparam_path())
 * is intentionally not backported here: it needs filename_lookup(), which
 * is static in fs/namei.c in this tree, and nothing this backport adds
 * (erofs included) uses fsparam_path()/fsparam_bdev() -- erofs identifies
 * its block device via fc->source/get_tree_bdev() instead.
 */

int fs_param_bad_value(struct fs_context *fc, struct fs_parameter *param)
{
	return invalfc(fc, "Bad value for '%s'", param->key);
}

int fs_param_is_bool(struct fs_context *fc, const struct fs_parameter_spec *p,
...
int fs_param_is_u32(struct fs_context *fc, const struct fs_parameter_spec *p,
		    struct fs_parameter *param, struct fs_parse_result *result)
{
...
int fs_param_is_s32(struct fs_context *fc, const struct fs_parameter_spec *p,
		    struct fs_parameter *param, struct fs_parse_result *result)
{
	if (param->type != fs_value_is_string ||
...
int fs_param_is_u64(struct fs_context *fc, const struct fs_parameter_spec *p,
		    struct fs_parameter *param, struct fs_parse_result *result)
{
	if (param->type != fs_value_is_string ||
...
int fs_param_is_string(struct fs_context *fc, const struct fs_parameter_spec *p,
		       struct fs_parameter *param, struct fs_parse_result *result)
{
...
int fs_param_is_blob(struct fs_context *fc, const struct fs_parameter_spec *p,
		     struct fs_parameter *param, struct fs_parse_result *result)
{
...
int fs_param_is_fd(struct fs_context *fc, const struct fs_parameter_spec *p,
		  struct fs_parameter *param, struct fs_parse_result *result)
{
	switch (param->type) {
...
int fs_param_is_blockdev(struct fs_context *fc, const struct fs_parameter_spec *p,
		  struct fs_parameter *param, struct fs_parse_result *result)
{
	return 0;
}
...
int fs_param_is_path(struct fs_context *fc, const struct fs_parameter_spec *p,
		     struct fs_parameter *param, struct fs_parse_result *result)
{
	return 0;
}
