			     ...
			     struct erofs_device_info *dif, erofs_off_t *pos)
{
	...
	if (erofs_is_fscache_mode(sb)) {
		...
		if (IS_ERR(bdev))
			return PTR_ERR(bdev);
		dif->bdev = bdev;
		dif->dax_dev = NULL;	/* DAX is not supported in this tree */
	}
...
static int erofs_fc_fill_super(struct super_block *sb, struct fs_context *fc)
{
	...
	if (erofs_is_fscache_mode(sb)) {
		...
		sbi->dax_dev = NULL;	/* DAX is not supported in this tree */
	}
...
/*
 * This tree's do_remount_sb() doesn't dispatch to fs_context ->reconfigure(),
 * so keep a legacy remount hook that just enforces read-only.
 */
