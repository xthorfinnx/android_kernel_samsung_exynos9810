 ...
 */
static int z_erofs_cache_releasepage(struct page *page, gfp_t gfp)
{
	return z_erofs_cache_release_folio(page_folio(page), gfp);
}

static void z_erofs_cache_invalidatepage(struct page *page,
					 unsigned int offset,
					 unsigned int length)
{
	z_erofs_cache_invalidate_folio(page_folio(page), offset, length);
}
				       ...
				       int bios)
{
	...
	if (!in_task() || irqs_disabled() || rcu_preempt_depth()) {
#ifdef CONFIG_EROFS_FS_PCPU_KTHREAD
		struct kthread_worker *worker;
